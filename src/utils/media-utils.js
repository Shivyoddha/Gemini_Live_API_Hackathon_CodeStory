...
export class AudioStreamer {
  ...
  /**
   * Mute mic — keeps the hardware stream open but stops forwarding audio to Gemini.
   * Call this while the agent is speaking to prevent VAD false-positives.
   */
  mute() {
    this.isStreaming = false;
    console.log("🔇 Microphone muted (VAD protection)");
  }

  /**
   * Unmute mic — resumes forwarding audio to Gemini (stream must already be started).
   * Call this when ready to accept user speech.
   */
  unmute() {
...
export class AudioPlayer {
  ...
  /**
   * Tap the gainNode output into a MediaStreamDestinationNode so the audio
   * can be fed into a MediaRecorder for recording.  The gainNode still drives
   * the speakers normally — this just adds an extra branch.
   * @returns {MediaStream|null}
   */
  createRecordingStream() {
    if (!this.isInitialized || !this.gainNode || !this.audioContext) {
      console.warn('[AudioPlayer] Cannot create recording stream: not initialized');
      return null;
    }
    if (this._recordingDest) return this._recordingDest.stream;
    this._recordingDest = this.audioContext.createMediaStreamDestination();
    this.gainNode.connect(this._recordingDest);
    console.log('[AudioPlayer] Recording stream created');
    return this._recordingDest.stream;
  }

  /**
   * Disconnect and remove the recording tap.
   */
  destroyRecordingStream() {
    if (this._recordingDest && this.gainNode) {
      try { this.gainNode.disconnect(this._recordingDest); } catch (_) { /* already disconnected */ }
      this._recordingDest = null;
      console.log('[AudioPlayer] Recording stream destroyed');
    }
  }
